...
def parse_xml(xml_file):
   ...
   # print 'xml is %s'%xml_file
    for item in root:
        if item.tag == 'folder':
            if item.text == 'n02814533':
                obj_name = "light_vehicle"
    #        if item.text == 'n03769881':
     #           obj_name = "heavy_vehicle"
            if item.text == 'n02835271':
                obj_name = "two_wheeler"
          ...
          #      obj_name = "light_vehicle"
	    if item.text == 'n03790512':
                obj_name = "two_wheeler"
            if item.text == 'n03791053':
                obj_name = "two_wheeler"
            if item.text == 'n03792782':
                obj_name = "two_wheeler"
            if item.text == 'n04037443':
                obj_name = "light_vehicle"
            if item.text == 'n04146614':
                obj_name = "heavy_vehicle"
            if item.text == 'n04285008':
                obj_name = "light_vehicle"
            if item.text == 'n04487081':
                obj_name = "heavy_vehicle"
        if item.tag == 'filename':
          image_path = os.path.join(DATA_PATH, 'VOC2007/JPEGImages', item.text + '.JPEG')
