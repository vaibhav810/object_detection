...
class YoloSolver(Solver):
  ...
  def solve(self):
    ...
    saver2 = tf.train.Saver(self.net.trainable_collection, write_version=2)
    ...
    for step in xrange(self.max_iterators):
      ...
      if step % 10 == 0:
        ...
        print(format_str % (datetime.now(), step, loss_value,
                                         examples_per_sec, sec_per_batch), file=open("output_network_loss.txt", "a"))
        sys.stdout.flush()
      if step % 100 == 0:
      ...
      if step % 100 == 0:
        saver2.save(sess, self.train_dir + 'model1_final_224_5640.ckpt', global_step=step)
    sess.close()
